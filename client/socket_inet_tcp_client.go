...
func main() {
    ...
    fmt.Printf("AF_INET TCP client, pid %d\n", os.Getpid())
    ...
    // 開始連線.
    tmpConnRD, cErr = net.DialTimeout(tcpNet, tcpLocalAddr, netTimeout)
    if cErr != nil {
        fmt.Printf("call net.DialTimeout(%s, %s, %v) fail [%s]\n",
                   tcpNet, tcpLocalAddr, netTimeout, cErr.Error())
        return
    }
    remoteConnRD = tmpConnRD.(*net.TCPConn)
